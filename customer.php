...
<html>
	...
	<body>
		...
		<ul> 
			...
			<li>Membership Number: <?= (int)$_GET["membership"] ?> </li>
			<?php
				$way = array();
				if (isset($_GET["organic"])) {
					array_push($way,"Organic");
				}
				if (isset($_GET["domestically"])) {
					array_push($way,"Domestically Produced");
				}
				if (isset($_GET["genetically"])) {
					array_push($way,"Genetically Modified");
				}
				if (isset($_GET["newly"])) {
					array_push($way,"Newly Harvested");
				}
			?>
			<li>Options: 
			<?php for($i = 0; $i < count($way); $i++) {?>
				<?= $way[$i]?>
			<?php }?>
			</li>
						
			<li>Fruits: <?= $_GET["fruits"] ?></li>
			<li>Credit <?= (int)$_GET["cardnum"] ?>(<?= $_GET["cardtype"] ?>)</li>
		</ul>
